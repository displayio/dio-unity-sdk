...
{
    /// <summary>
    /// An infeed ad for one placement, created via <see cref="DioAds.Infeed"/>.
    /// Load fetches the ad, Show inserts it on screen, Hide removes it again.
    /// The size comes from the placement — the publisher chooses the position, not the size.
    /// </summary>
    public class DioInfeedAd : IDioInlineAdCallbacks
    {
        public string PlacementId { get; }

        /// <summary>Where the ad sits. Applied immediately when already shown.</summary>
        ...
        /// points on iOS. X moves the ad away from the left edge, or from the right edge for
        ...
        /// <summary>
        /// Which axes keep the ad clear of notches and system bars. Defaults to both;
        /// set it per axis to pin the ad to a physical screen edge.
        /// </summary>
        ...
        internal DioInfeedAd(string placementId)
        {
            PlacementId = placementId;
#if UNITY_ANDROID && !UNITY_EDITOR
            impl = new DioAndroidInlineAd(placementId, DioAdUnitType.Infeed, this);
#elif UNITY_IOS && !UNITY_EDITOR
            DioIosBridge.RegisterInline(placementId, DioAdUnitType.Infeed, this);
#endif
        }
        ...
        private void ApplyLayout()
        {
            ...
            ((IDioInlineAdCallbacks)this).ShowFailed(
                DioAdInfoFactory.Minimal(PlacementId, DioAdUnitType.Infeed),
                new DioError("display.io ads are only available on Android"));
#endif
        }
        ...
        void IDioInlineAdCallbacks.NoFill(DioError error) =>
            OnNoFill?.Invoke(DioAdInfoFactory.Minimal(PlacementId, DioAdUnitType.Infeed));

        void IDioInlineAdCallbacks.LoadFailed(DioError error) =>
            OnLoadFailed?.Invoke(DioAdInfoFactory.Minimal(PlacementId, DioAdUnitType.Infeed), error);
